...
public class HardGameScreen {

    private static final String[] WORDS = {"apple.", "banana!", "orange?", "grape,", "pear;", "melon:", "strawberry'", "kiwi-", "peach(", "plum)", "12345", "67890", "!@#$%", "^&*()"};

    public static Scene createHardGameScene() {
        VBox hardGameVBox = new VBox(20);
        hardGameVBox.setAlignment(Pos.CENTER);
        hardGameVBox.setStyle("-fx-background-color: #2b2b2b;");
        ...
        backButton.setOnAction(e -> {
            // Create and start the initial scene of TypingApp
        ...
        Scene scene = new Scene(hardGameVBox, 600, 400);

        scene.setOnKeyTyped(event -> {
            String typedCharacter = event.getCharacter();
            if (typedCharacter.matches("[a-zA-Z0-9.,!?';:\\-()@#$%^&*]")) {
                updateWordLabels(wordLabels, typedCharacter);
                if (isAllWordsTyped(wordLabels)) {
                    CompletionMessageHandler.showCompletionMessage(hardGameVBox, startTime[0]);
                }
                if (startTime[0] == 0) {
        ...
        hardGameVBox.getChildren().addAll(topRightBox, wordsFlowPane);
